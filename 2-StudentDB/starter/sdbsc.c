...
int get_student(int fd, int id, student_t *s){
    if (s == NULL || id < MIN_STD_ID) {
        fprintf(stderr, M_ERR_STD_PRINT);
        return ERR_DB_OP;
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    if (read(fd, &s, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (s.id != 0 && s.id != DELETED_STUDENT_ID) {
...
int compress_db(int fd){
    int temp_fd = open(TMP_DB_FILE, O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (temp_fd < 0) {
        fprintf(stderr, M_ERR_DB_CREATE);
        return ERR_DB_FILE;
    }
